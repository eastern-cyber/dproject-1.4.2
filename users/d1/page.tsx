...
// Interface for D1 table data
interface D1Data {
  id: number;
  user_id: string;
  rate_thb_pol: number | string; // Allow both number and string
  append_pol: number | string;   // Allow both number and string
...
type SortField = 'user_id' | 'rate_thb_pol' | 'append_pol' | 'append_pol_date_time' | 'created_at';
type SortDirection = 'asc' | 'desc';

export default function D1Dashboard() {
  const [d1Data, setD1Data] = useState<D1Data[]>([]);
  const [mounted, setMounted] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  const [sortField, setSortField] = useState<SortField>('created_at');
  const [sortDirection, setSortDirection] = useState<SortDirection>('desc');
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedRecord, setSelectedRecord] = useState<D1Data | null>(null);
  ...
  const fetchD1Data = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch('/api/d1-data');
      ...
      setError(error instanceof Error ? error.message : 'Failed to fetch D1 data');
      setD1Data([]);
    } finally {
      setLoading(false);
    }
  ...
  const downloadCSV = async () => {
    try {
      ...
      let dataToDownload = d1Data;
      
      // If we have filtered data, fetch all data for complete export
      if (searchTerm) {
        const response = await fetch('/api/d1-data');
        if (!response.ok) {
          throw new Error('Failed to fetch D1 data for export');
        }
        const data = await response.json();
        dataToDownload = data || [];
      }
      
      // Define CSV headers for D1 data
      const headers = [
        'ID',
        'User ID',
        'Rate THB/POL',
      ...
      link.setAttribute('download', `d1_data_export_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const filteredData = useMemo(() => {
    if (!d1Data || d1Data.length === 0) return [];
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(record => {
        ...
        return userIdMatch || txHashMatch;
      });
    }
  ...
  // Helper function to safely calculate total append POL from D1 data
  const calculateTotalAppendPOL = (): string => {
    if (!d1Data || d1Data.length === 0) return '0.00';
    
    const total = d1Data.reduce((sum, record) => {
  ...
  // Helper function to safely calculate average rate from D1 data
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md mb-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex-1">
            <input
              type="text"
              placeholder="Search D1 records by user ID or transaction hash..."
              value={searchTerm}
          ...
          <div className="flex items-center gap-4">
            ...
            {/* <button
              onClick={downloadCSV}
              disabled={downloading || d1Data.length === 0}
      ...
      {/* D1 Data Table */}
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md mb-6">
        <h2 className="text-xl font-semibold mb-4">D1 Records</h2>
        
        {!loading && d1Data.length === 0 && (
          <div className="text-center py-8">
            <p className="text-gray-500 dark:text-gray-400 mb-4">No D1 data found in the database</p>
            <button
              onClick={handleRetry}
          ...
          >
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold">D1 Record Details</h2>
                <button
                  onClick={() => setSelectedRecord(null)}
